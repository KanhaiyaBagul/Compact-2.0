...
toggleMode.addEventListener('click', () => {
  ...
  document.getElementById('subtitle').textContent = isSignUp ? 'Start your journey with CodePlus-AI' : 'Sign in to continue to CodePlus-AI';
...
btnSubmit.addEventListener('click', async () => {
  ...
  try {
    ...
    console.log('[Auth] Done. Redirecting or closing...');
...
btnGoogle.addEventListener('click', async () => {
  ...
  try {
    ...
    console.log('[Auth] Google Success. Redirecting or closing...');
    const urlParams = new URLSearchParams(window.location.search);
    const returnTo = urlParams.get('returnTo');
    if (returnTo && returnTo.startsWith('http')) {
      window.location.href = returnTo;
