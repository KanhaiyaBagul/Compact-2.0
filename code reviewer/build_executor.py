...
class BuildExecutor:
    ...
    def run_build(self, capture_output: bool = True, show_live: bool = False) -> BuildResult:
        """
        Run the build command and parse results.
        
        Args:
            capture_output: If True, capture stdout/stderr for parsing
            show_live: If True, also print output to the console as it happens
        ...
        try:
            if show_live and capture_output:
                ...
                try:
                    returncode = process.wait(timeout=self.config.build_timeout)
                    t1.join()
                    t2.join()
                ...
                # Create a pseudo-CompletedProcess for the rest of the logic
