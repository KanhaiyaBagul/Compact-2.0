...
class ReviewSession:
    ...
    def get_progress_summary(self) -> str:
        ...
        if self.current_directory:
            lines.append(f"[DIR] Directory: {self.current_directory}")
        ...
        if self.current_file:
            lines.append(f"[FILE] File: {self.current_file}")
        ...
        if self.changed_files:
            lines.append(f"[EDIT] Edits: {len(self.changed_files)} files modified")
...
class SecretScanner:
    ...
    def format_findings(cls, findings: List[Tuple[str, str, str]]) -> str:
        ...
        lines = [
            "=" * 70,
            "WARNING: POTENTIAL SECRETS DETECTED IN COMMIT",
            "=" * 70,
            "",
...
class ReviewLoop:
    ...
    def __init__(
        ...
        if not is_valid:
            ...
            print(f"    [OK] Persona validated: {persona_dir.name}")
...
WORKFLOW:
1. Read REVIEW-SUMMARY.md to see completed directories (marked with [OK])
...
Output ONLY the lesson entry, nothing else."""
    ...
    def _update_review_summary(self, changed_files: List[str], commit_message: str,
        ...
        entry = f"""
## {timestamp} - {directory or 'Build'} [OK]
...
"""
    ...
    def _check_for_loop(self, action: Dict[str, Any]) -> Optional[str]:
        ...
        if self.session.consecutive_identical_actions >= MAX_CONSECUTIVE_RECOVERY:
            ...
            if action_type == 'READ_FILE':
                ...
                return (
                    f"\n{'='*70}\n"
                    f"!!! INFINITE LOOP WARNING (attempt {self.session.consecutive_identical_actions}/{MAX_CONSECUTIVE_RECOVERY}) !!!\n"
                    f"{'='*70}\n\n"
    ...
    def _execute_action(self, action: Dict[str, Any]) -> str:
        ...
        if action_type == 'FIND_FILE':
            ...
            if has_makefile:
                result += f"[OK] Directory has Makefile - valid source module\n\n"
            else:
            ...
            if chunker.should_chunk(path):
                ...
                header = (
                    f"\n[REPORT] CHUNKED FILE REVIEW MODE\n"
                    f"   File: {path}\n"
            ...
            if line_count > 500 or file_size > 20000:
                warning = (
                    f"\nWARNING:  NOTE: Medium-sized file ({line_count} lines, {file_size} bytes)\n"
            ...
            if success:
                ...
                if self.session.edit_failure_count >= MAX_EDIT_FAILURES:
                    ...
                    return (
                        ...
                        f"{'='*70}\n"
                        f"WARNING:  EDIT FAILURE LOOP DETECTED WARNING:\n"
                        f"{'='*70}\n\n"
...
""")
    ...
    def _recover_from_loop(self, action: Dict[str, Any]) -> str:
        ...
        print("\n" + "="*70)
        print("!!! AUTOMATIC LOOP RECOVERY INITIATED !!!")
        print("="*70)
        ...
        print("="*70)
        print("[OK] RECOVERY COMPLETE - You must now take a different approach")
        print("="*70 + "\n")
        
        # Build recovery message for AI
        recovery_msg = (
            f"\n{'='*70}\n"
            f"[RECOVERY] AUTOMATIC RECOVERY COMPLETED\n"
            f"{'='*70}\n\n"
    ...
    def _validate_response(self, response: str) -> Optional[str]:
        ...
        for check_fn, message in truncation_indicators:
            if check_fn(response):
                return (
                    f"\nWARNING:  INCOMPLETE RESPONSE DETECTED\n\n"
                    f"Problem: {message}\n"
        ...
        while (self.forever_mode or
            ...
            try:
                ...
                if is_recoverable and self.forever_mode:
                    ...
                    print(f"\n{'='*60}")
                    print(f"WARNING:  RECOVERABLE ERROR (attempt {self.session.llm_retry_count})")
                    print('='*60)
            ...
            if not action:
                ...
                if self.session.consecutive_parse_failures >= 5:
                    ...
                    recovery_msg = (
                        f"\n{'='*70}\n"
                        f"WARNING:  CRITICAL: UNPARSEABLE LOOP DETECTED WARNING:\n"
                        f"{'='*70}\n\n"
...
def preflight_sanity_check(
    ...
    try:
        ...
        result = builder.run_build(capture_output=True, show_live=True)
...
Examples:
    ...
    # Run Risk Prediction Engine (Analysis of Commit History)
    if review_config.get('enable_risk_analysis', True):
        print("\n*** Starting Risk Prediction analysis (Commit History)...")
        risk_engine = RiskEngine(source_root, llm_client)
        risk_report = risk_engine.analyze_risk()
        
        print(f"    [!] PROJECT RISK SCORE: {risk_report['score']}/10.0")
        print(f"    [!] REASONING: {risk_report['reasoning']}")
        if risk_report.get('hotspots'):
            print("    [!] RECENT HOTSPOTS:")
            for hs in risk_report['hotspots'][:3]:
                 print(f"      * {hs['path']} ({hs['changes']} changes)")
        print()
