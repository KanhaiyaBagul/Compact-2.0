...
def run_command(label: str, argv: list[str], log_file) -> int:
    ...
    if sys.platform == 'win32':
        ...
        try:
            ...
            echo(log_file, f"*** {label} failed to start: {e}")
            return 1
    else:
...
def main() -> int:
    ...
    with log_path.open("ab") as log_file:
        ...
        if sys.platform == 'win32':
            if not Path(venv).exists() and Path(".venv").exists():
                venv = ".venv"
        ...
        if sys.platform == 'win32':
            echo(log_file, "Windows detected: skipping make check-deps")
            exit_code = 0
        else:
        ...
        if not config_path.exists():
            ...
            if sys.platform == 'win32':
                if defaults_path.exists():
                    shutil.copy(str(defaults_path), str(config_path))
                    echo(log_file, "✓ Created config.yaml from sample config.")
                else:
                    echo(log_file, "*** ERROR: config.yaml.sample not found; cannot initialize config.yaml")
                    return 1
            else:
                bash = resolve_bash()
                if not bash:
            ...
            if sys.platform == 'win32':
                exit_code = run_command("config-update", [venv_py, "scripts/config_update.py"], log_file)
            else:
