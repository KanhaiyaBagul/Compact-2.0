...
def create_client_from_config(config_dict: Dict[str, Any]) -> LLMClient:
    ...
    quantization = str(llm_cfg.get("quantization") or "").lower().strip()

    # Apply quantization technique to model name if specified
    if quantization == "8bit" and model and ":" in model and "-q" not in model:
        model = f"{model}-q8_0"
        logger.info(f"Applied 8-bit quantization technique: using model {model}")
